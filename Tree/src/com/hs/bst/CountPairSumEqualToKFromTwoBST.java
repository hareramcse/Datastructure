...
public class CountPairSumEqualToKFromTwoBST {
	...
	private static int countPairs(Node root1, Node root2, int x) {
		...
		while (true) {
			...
			// move to next possible node in the inoder traversal of BST 1
			else if ((top1.data + top2.data) < x) {
				st1.pop();
				root1 = top1.right;
			}

			// move to next possible node in the reverse inoder traversal of BST 2
			else {
				st2.pop();
				root2 = top2.left;
			}
